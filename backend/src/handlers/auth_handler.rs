...
use crate::repositories::{SqlxSessionRepository, SessionRepository, UserRepository};
use crate::AppState;

use uuid::Uuid;

pub(crate) async fn resolve_authenticated_user_id(
    state: &Arc<AppState>,
    headers: &HeaderMap,
    session_repo: &SqlxSessionRepository,
    ...
    let token_data = match jsonwebtoken::decode::<crate::models::Claims>(
        &token,
        &DecodingKey::from_secret(state.jwt_secret.as_ref()),
        &Validation::default(),
    ) {
        Ok(d) => d,
        Err(_) => return Err((StatusCode::UNAUTHORIZED, Json(json!({ "error": "Invalid session" }))).into_response()),
    };

    let sid = match token_data.claims.sid {
        Some(s) => s,
        None => return Err((StatusCode::UNAUTHORIZED, Json(json!({ "error": "Invalid session" }))).into_response()),
    };
    ...
    match session_repo.find_by_id(session_id).await {
        Ok(Some(s)) => {
            if s.revoked || s.expires_at < chrono::Utc::now() {
                return Err((StatusCode::UNAUTHORIZED, Json(json!({ "error": "Session invalid" }))).into_response());
            }
            ...
            if expected != s.fingerprint {
                ...
                return Err((StatusCode::UNAUTHORIZED, Json(json!({ "error": "Fingerprint mismatch" }))).into_response());
            }
        ...
        Ok(None) => Err((StatusCode::UNAUTHORIZED, Json(json!({ "error": "Session not found" }))).into_response()),
...
pub async fn me_handler(
    ...
    match user_repo.find_by_id(user_id).await {
        Ok(Some(user)) => {
            let public_user = crate::models::PublicUser::from(user);
            let body = Json(json!({
                "authenticated": true,
                "user_id": public_user.id,
                "name": public_user.name,
                "pseudo": public_user.pseudo,
